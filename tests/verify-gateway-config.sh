...
require_text "$config" "location = /api/"
require_text "$config" "location ~ ^/(actuator|internal)(/|$)"
...
require_text "$compose" "USERS_SERVICE_URL: http://usuarios-service:8081"
require_text "$compose" "PRODUCTS_SERVICE_URL: http://productos-service:8082"
